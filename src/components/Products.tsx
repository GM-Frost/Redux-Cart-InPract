...
const Products: React.FC = () => {
  ...
  const { currentData: products, isLoading } = useGetProductsQuery(1);

  const addCart = (product: IProductsType) => {
    dispatch(addToCart(product));
  };

  const cards = products?.map((product: IProductsType) => (
    <div
      key={product.id}
      className="my-1 px-1 w-full md:w-1/2 lg:my-4 lg:px-4 lg:w-1/3"
    >
      <div className=" h-full w-full max-w-sm bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700">
        <img
          className="rounded-t-lg block  w-full object-fit h-80"
          src={product.image}
          alt="product image"
        />
